...
look things up in :mod:`mcuhome.model.registry` without re-checking that they
...
from mcuhome.model.errors import ConfigError, ErrorCollector
...
def resolve(config: RawConfig) -> DeviceModel:
    """Stage 3. Expects a configuration that passed :func:`~mcuhome.workbench.validate.validate`."""
...
def _resolve_pairing(config: RawConfig, matter_enabled: bool) -> PairingModel | None:
    ...
    if (
        ...
        # Unreachable through the CLI: mcuhome.workbench.validate refuses this
...
def _resolve_build(
    ...
    if network.matter_enabled:
        ...
        kconfig += [
            ...
            # group emitted by mcuhome.model.pairing.kconfig_lines() straight
            # into the fragment (mcuhome/compiler/generate.py), so that no code
