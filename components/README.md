...
| Component | Kind | Role |
|---|---|---|
| `matter/` | core | The Matter runtime: bring-up sequence, generated-tables → ember translation, external attribute callbacks, DNS-SD retry service (ADR 0014), and — behind `MCUHOME_MATTER_OTA`, which a board's update scheme turns on — the OTA Requestor wiring plus MCUHome's own image processor and image-header parser (ADR 0015 decision 5). The only place in the tree that includes a CHIP header. |
