...
def test_the_package_directory_maps_onto_the_shared_tree() -> None:
    ...
    (``containers/builder/run``) and ``bin/generate`` inserts it into
...
def test_the_container_launcher_execs_the_module_that_holds_the_abi() -> None:
    """``containers/builder/run`` is image content and names an import path.
    ...
    """
    launcher = (REPO_ROOT / "containers" / "builder" / "run").read_text(encoding="utf-8")
...
# --------------------------------------------------------------------------
# The `remote` extra of `mcuhome-workbench`
# --------------------------------------------------------------------------
#
# These two live here, and not beside the session-client tests they are
# about, for one reason: that file is gated on the extra plus a sibling
# checkout of the build server, and a guard against a silently dropped
# extra is worth nothing in the only environment that would notice —
# the one where the extra is not installed. They need neither dependency
# (`sessionclient` imports nothing optional at module level), so they
# belong where the rest of the packaging invariants are.
