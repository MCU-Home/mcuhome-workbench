...
def test_secret_is_resolved(write_config) -> None:
    ...
    data = load_config(entry, secrets_file=entry.parent / "secrets.yaml")
...
def test_unknown_secret_points_at_the_tag(write_config) -> None:
    ...
    with pytest.raises(ConfigError) as caught:
        load_config(entry, secrets_file=entry.parent / "secrets.yaml")

    error = caught.value
    assert error.message == 'There is no secret called "device_label" in secrets.yaml.'
...
def test_missing_secrets_file_is_explained(write_config) -> None:
    ...
    with pytest.raises(ConfigError) as caught:
        load_config(entry, secrets_file=entry.parent / "secrets.yaml")
    assert "there is no secrets.yaml to read it from" in caught.value.message
...
def test_secrets_file_is_only_read_when_used(write_config) -> None:
    ...
    # No secrets.yaml exists, and none is needed.
    data = load_config(entry, secrets_file=entry.parent / "secrets.yaml")
