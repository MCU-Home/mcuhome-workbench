...
"""Commissioning credentials: the math and the atomicity.

:mod:`mcuhome.model.pairing` is the subject, together with the one
consumer that must never emit half of what it says
(:mod:`mcuhome.compiler.generate`). The command that draws credentials
into a user's YAML is :mod:`mcuhome.workbench.provision`, tested in
``test_pairing_workbench.py``.
